async function postData () {
    ...
    try {
      const response = await axios.post("/UPLOADER/mark_attachment_upload")
...
async function upload() {
    ...
    const key = file.name;
    const response = await axios.get(`/oss/uploader/session_attachment/sign?key=${key}&type=${file.type}`);
